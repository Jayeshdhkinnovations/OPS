...
export const appName = 'SignToowix';
...
// database, all reachable under this same origin at /app/<slug> instead of
// each company needing its own container/port. See cloud/multiTenant.js.
//
// PUBLIC_ORIGIN is the address the outside world actually reaches this
...
// Where the SuperAdmin control-plane's Company records live - this backend
// reads that collection directly (read-only) to know which companies to
// mount, both on startup and whenever a new one is added live.
export const superAdminMongoUri =
  process.env.SUPERADMIN_MONGODB_URI || 'mongodb://mongo:27017/SuperAdminDB';

// Shared secret SuperAdminServer must present when calling this server's
// internal /admin/mount-company endpoint - keeps that endpoint from being
// callable by anyone who can merely reach this container's network.
export const internalAdminSecret = process.env.INTERNAL_ADMIN_SECRET;
