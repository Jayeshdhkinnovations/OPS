...
export default async function addUser(request) {
  ...
  if (name && email && password && organization && team && role && tenantId) {
    try {
      ...
      // Enforce this Tenant's MaxUsers cap (set by the Super Admin Console).
      // Checked server-side, against the Tenant's own stored limit, so it
      // cannot be bypassed by calling this function directly.
      const tenantQuery = new Parse.Query('partners_Tenant');
      const tenant = await tenantQuery.get(callerTenantId, { useMasterKey: true });
      const maxUsers = tenant.get('MaxUsers');
      if (typeof maxUsers === 'number') {
        const userCountQuery = new Parse.Query('contracts_Users');
        userCountQuery.equalTo('TenantId', {
          __type: 'Pointer',
          className: 'partners_Tenant',
          objectId: callerTenantId,
        });
        userCountQuery.notEqualTo('IsDisabled', true);
        const currentUserCount = await userCountQuery.count({ useMasterKey: true });
        if (currentUserCount >= maxUsers) {
          throw new Parse.Error(
            Parse.Error.OPERATION_FORBIDDEN,
            `User limit reached (${maxUsers}). Contact your administrator to increase this limit.`
          );
        }
      }
